...
public class UserControllerTest {
    ...
    void registrationTest() throws Exception {
        ...
        MockMultipartFile jsonFile = new MockMultipartFile("registerBeanString", "", "application/json",
                getFileContent("controller/user/registration-request.json").getBytes());
        ...
        assertThat(userRepository.findUsersByEmail("email").size()).isEqualTo(1);
        assertThat(userRepository.findUsersByEmail("email").get(0).isEnabled()).isFalse();
    }

    @Test
    void registrationWhenThisEmailAlreadyUseTest() throws Exception {
        User user = userService.registerUser(buildRegisterBean());
        user.setEnabled(true);
        userRepository.save(user);
        MockMultipartFile jsonFile = new MockMultipartFile("registerBeanString", "", "application/json",
    ...
    @Test
    void registrationWithIncorrectRequestTest() throws Exception {
        when(javaMailSender.createMimeMessage()).thenReturn(new MimeMessage((Session) null));

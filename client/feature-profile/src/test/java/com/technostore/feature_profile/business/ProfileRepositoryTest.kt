...
class ProfileRepositoryTest {
    ...
    /* get profile */
    @Test
    fun `profile with 200 status → return success with profile model`() = runTest {
        UserServiceMock {
            profile.success()
        }
    ...
    @Test
    fun `changePassword return internal error → return error`() = runTest {
        UserServiceMock {
    ...
    @Test
    fun `changePassword return 404 code → return wrong password error`() = runTest {
        UserServiceMock {
    ...
    /* edit profile */
    @Test
    fun `edit profile return success, byteArray is not empty → return success`() = runTest {
        UserServiceMock {
            editProfile.success()
        }
    ...
    @Test
    fun `edit profile return success, byteArray is empty →  return success`() = runTest {
        UserServiceMock {
            editProfile.success()
        }
        val result = profileRepository.editProfile(
    ...
    @Test
    fun `registration return internal error, byteArray is not empty → return error`() = runTest {
        UserServiceMock {
    ...
    @Test
    fun `registration return internal error, byteArray is empty → return error`() = runTest {
        UserServiceMock {
            editProfile.internalError()
        }
        val result = profileRepository.editProfile(
    ...
    /* logout */
    @Test
    fun `logout return success → clear local data`() = runTest {
        UserServiceMock {
            logout.success()
        }
    ...
    @Test
    fun `logout with empty refresh token return success → clear local data`() = runTest {
        UserServiceMock {
            logout.success()
        }
        appStore.apply {
